function y=qam32(N_code,fc,fs,fd,Ac)
    ...
    %xlabel('比特序列');
    %ylabel('信号幅度');
    x4=reshape(x,k,length(x)/k);%将原始的二进制比特序列每四个�?组分组，并排列成k行length(x)/k列的矩阵 
    xsym=bi2de(x4.','left-msb');%将矩阵转化为相应�?64进制信号序列 
   % figure(2);
    %stem(xsym);%画出相应�?16进制信号序列 
    %title('64进制随机信号');
    %xlabel('信号序列');
   % ylabel('信号幅度');
    %y=modulate(modem.qammod(M),xsym);%�?64QAM调制器对信号进行 
    y=qammod(xsym,M);
